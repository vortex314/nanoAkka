...
Remote::Remote(Thread& thr) : Actor(thr),
	...
	_measureTimer(thr,1,500,true),
	_leftButton( _buttonLeft,buttonLeft),
	_rightButton( _buttonRight,buttonRight) {

	ledLeft.on(false);
	ledRight.on(false);
	ledLeft.sync([&](bool b) {_ledLeft.write(b?0:1);});
	ledRight.sync([&](bool b) {_ledRight.write(b?0:1);});

	_measureTimer >> *(new Sink<TimerMsg,2>([&](const TimerMsg& tm) {
		buttonLeft.request();
		buttonRight.request();
		potLeft = _adcLeft.getValue();
		potRight = _adcRight.getValue();
	}));
}
