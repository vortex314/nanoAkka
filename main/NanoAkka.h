...
class PointerQueue {
	...
	public :
		...
		int read(voidPtr* ptr) {
			uint32_t readPtr = _readPtr.fetch_add(1, std::memory_order_relaxed);
		...
		int write(voidPtr ptr) {
			uint32_t writePtr = _writePtr.fetch_add(1,std::memory_order_relaxed);
